...
public class BattleScreen extends JPanel {
    ...
    private void setupAbilityButtons() {
        ...
        ultimateBtn.setBorder(BorderFactory.createLineBorder(Color.WHITE, 2));import javax.swing.*;
import java.awt.*;
import java.util.Random;
import java.net.URL;
...
public class BattleScreen extends JPanel {
    ...
    public BattleScreen(GameWindow window) {
        this.window = window;
        this.pet = window.getGameState().getCurrentPet();
        this.petLevel = window.getGameState().getPetLevel();
        this.enemy = StoryManager.getChapterEnemy();
    ...
    private void setupUI() {
        ...
        int imageSize = GameConstants.BATTLE_IMAGE_SIZE;
        ...
        if (enemy.isBoss()) {
            bossWarningLabel = new JLabel("BOSS BATTLE!", SwingConstants.CENTER);
            bossWarningLabel.setBounds(280, 50, 240, 30);
            bossWarningLabel.setFont(new Font("Arial", Font.BOLD, 20));
            bossWarningLabel.setForeground(Color.RED);
            add(bossWarningLabel);
    ...
    private void useAbility(String abilityType) {
        ...
        String result = switch (abilityType) {
            ...
            case "skill" -> {
                if (pet.getMana() < pet.getSkillManaCost()) {
                    yield "Not enough mana! Need " + pet.getSkillManaCost() + " mana for " + pet.getSkillName() + ".";
                }
                yield pet.useSkill(enemy);
            }
            case "ultimate" -> {
                if (pet.getMana() < pet.getUltimateManaCost()) {
                    yield "Not enough mana! Need " + pet.getUltimateManaCost() + " mana for " + pet.getUltimateName() + ".";
                }
    ...
    private void handleVictory() {
        ...
        if (enemy.isBoss()) {
            victoryMessage = "<html><div style='text-align: center; color: #90EE90;'>" +
                    ...
                    "+" + GameConstants.BOSS_VICTORY_HAPPINESS + " Happiness & +" + GameConstants.BOSS_VICTORY_EXP + " EXP</div></html>";
            pet.increaseHappiness(GameConstants.BOSS_VICTORY_HAPPINESS);
            petLevel.addExperience(GameConstants.BOSS_VICTORY_EXP);
        } else {
            victoryMessage = "<html><div style='text-align: center; color: #90EE90;'>" +
                    "VICTORY! You defeated " + enemy.getName() + "!<br>" +
                    "+" + GameConstants.NORMAL_VICTORY_HAPPINESS + " Happiness & +" + GameConstants.NORMAL_VICTORY_EXP + " EXP</div></html>";
            pet.increaseHappiness(GameConstants.NORMAL_VICTORY_HAPPINESS);
            petLevel.addExperience(GameConstants.NORMAL_VICTORY_EXP);
        }
        ...
        Timer victoryTimer = new Timer(3000, e -> {
            if (!StoryManager.isGameComplete()) {
                StoryManager.nextChapter();
                window.switchScreen(new StoryScreen(window));
            } else {
                JOptionPane.showMessageDialog(this,
                        "LEGENDARY! You completed CyberWood Chronicles!\n\n" +
                                "You defeated the Lab Guardian!\n" +
